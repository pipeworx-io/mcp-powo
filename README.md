...
`tools/list` at `https://gateway.pipeworx.io/powo/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/powo_search_plants \
  -H 'Content-Type: application/json' \
  -d '{"query":"Quercus robur"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/powo_search_plants`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
